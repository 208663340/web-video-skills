...
在 Codex / Codex CLI / Codex desktop 中使用本 Skill 时，先读
[`references/CODEX.md`](references/CODEX.md)，再按本文主流程执行。Codex 适配文件只负责
工具映射、Windows/PowerShell 脚手架注意事项、checkpoint 停顿方式和验证策略；本文仍是
内容、章节、主题和音频流程的主契约。
...
| [`references/CODEX.md`](references/CODEX.md) | Codex / Codex CLI / Codex desktop 中使用时先读 | Codex 工具映射、Windows shell 注意事项、checkpoint 策略、验证清单 |
