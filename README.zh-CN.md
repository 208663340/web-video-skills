...
如果在 Codex / Codex CLI / Codex desktop 里使用，先读 [`references/CODEX.md`](./references/CODEX.md)。它把 reviewer agent、checkpoint、shell 命令和验证步骤映射到 Codex 的工具习惯。
...
> 预览图来自上游 demo gallery。本仓库只保留 Codex 安装所需的 skill 文件。
...
- [CODEX.md](./references/CODEX.md)：Codex 专用工具映射、checkpoint、Windows 注意事项和验证清单
